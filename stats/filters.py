...
class PartyVotesFilter(django_filters.FilterSet):
    type = django_filters.ChoiceFilter(method='type_filter', choices=PartyVoteBehaviour.VOTING_TYPE_CHOICES)
    ...
    def type_filter(selfs, queryset, name, value):
        return queryset.filter(voting_type=value).distinct()

    def submitter_party_filter(self, queryset, name, value):
        return queryset.filter(submitter=value).distinct()

    def government_filter(self, queryset, name, value):
        return queryset.filter(government=value).distinct()

...
def create_verslagen_algemeen_overleg(year, max_n=None, skip_if_exists=False):
    ...
    for info in infos:
        try:
            ...
            if info['dossier_extra_id']:
                dossier_id += '-' + str(info['dossier_extra_id'])
            create_verslag(
...
@transaction.atomic
def create_verslag(overheidnl_document_id, dossier_id, kamerstuk_nr, skip_if_exists=False):
    if skip_if_exists and Kamerstuk.objects.filter(document__document_id=overheidnl_document_id).exists():
        return
    document = create_document(overheidnl_document_id, dossier_id=dossier_id)
